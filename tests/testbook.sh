...
$bindir/TICCL-indexer -t max --hash $outdir/TESTDP035.tsv.clean.anahash --charconf $datadir/nld.aspell.dict.clip20.ld2.charconfus --foci $outdir/TESTDP035.tsv.clean.corpusfoci
...
LC_ALL=C sort -t# $outdir/TESTDP035.tsv.clean.index > /tmp/index
diff /tmp/index $refdir/index > /dev/null 2>&1

if [ $? -ne 0 ]
then
    echo "differences in Ticcl-indexer results"
    echo "using diff /tmp/index $refdir/index"
    exit
fi

echo "start TICLL-LDcalc"

$bindir/TICCL-LDcalc --index $outdir/TESTDP035.tsv.clean.index --hash $outdir/TESTDP035.tsv.clean.anahash --clean $outdir/TESTDP035.tsv.clean --LD 2 -t max --artifrq 100000000 -o $outdir/TESTDP035.tsv.clean.ldcalc
...
$bindir/TICCL-rank -t 1 --alph $datadir/nld.aspell.dict.clip20.lc.chars --charconf $datadir/nld.aspell.dict.clip20.ld2.charconfus -o $outdir/TESTDP035.tsv.clean.ldcalc.ranked --debugfile $outdir/.TESTDP035.tsv.clean.ldcalc.debug.ranked --artifrq 0 --clip 5 --skipcols=10,11 $outdir/TESTDP035.tsv.clean.ldcalc 2> $outdir/.TESTDP035.RANK.stderr
...
diff $outdir/TESTDP035.tsv.clean.ldcalc.ranked $refdir/book.ranked >/dev/null 2>&1
if [ $? -ne 0 ]
then
